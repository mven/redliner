...
var redliner = (function redliner() {
  ...
  var path = require('path'),
      ...
      chai = require('chai'),
      expect = chai.expect,
      colorProps = require('color-props');
  ...
  return function(config) {
    ...
    // get all the .yml files from the config.redlines property
    var redlines = getRedlines();


    console.info('\nFound the following specifications of red lines:\n');
    redlines.map(key => { console.log(key); });
    console.info('\nRunning Tests...');
          ...
          return phantom.create().then(function (ph) {
            ...
            return ph.createPage().then(function (page) {
              ...
              return page.open(url).then(function (status) {
                ...
                // run js specific to the opened page
          ...
          describe('#'+testName, function() {
            ...
            Object.keys(test.testGroup).map(function(key) {
              ...
              it('should have ' + key + ': '+test.testGroup[key], function (done) {
                ...
                // identify if it's a color property,
                // then use colorProps to generate a color data object to test against
                if (/color/i.test(key)) {
                  test.testGroup[key] = colorProps(test.testGroup[key]).rgbCss;
                }
